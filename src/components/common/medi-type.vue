<template>
  <div>
      <el-button type="success" class="el-icon-circle-plus-outline" @click="dialogFormVisible=true;">新增药品类型</el-button>
<el-table
    :data="factory"
    border
...
<!-- 新增厂商弹框 -->
<el-dialog title="新增药品类型" :visible.sync="dialogFormVisible" class="factoty-dialog" :rules="addRules">
  <el-form :model="form" ref="addForm" :rules="addRules">
    <el-form-item label="药品类型名称"  prop="factoryName">
...
<!-- 修改厂商弹框 -->
<el-dialog title="修改药品类型" :visible.sync="dialogFormVisible1" class="factoty-dialog" :rules="addRules">
...
export default {
  ...
  data () {
    return {
     ...
     addRules:{
          factoryName:[{ required: true, message: '请输入药品类型', trigger: 'blur'  }
          ]
      }
    }
  },
  ...
  methods:{
      getType(){
           this.$http.get('medicine/getMedicineTypeList',
        {
          params: {
      ...
      console.log(val);
      this.$nextTick(this.getType());
    },
    deleteF(id){
     // alert(123);
        this.$confirm('确定删除此药品信息?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
        this.$http.delete(`medicine/deleteMedicineType/${id}`)
      .then( (response) => {
      console.log(response);
      if(response.data.statusCode===200){
       this.$message({
    ...
    submit(){
        this.$refs.addForm.validate((valid) => {
          ...
          this.$http.post('medicine/addMedicineType',
              {
    ...
    update(){
      this.$refs.addForm.validate((valid) => {
          if (valid) {
            //alert(this.addUser.roleId.id);
          this.$http.put('medicine/updateMedicineType',
              {
